...
- 每完成一个功能节点，先汇报已完成内容、验证结果和剩余风险，然后询问是否继续提交、推送或发布。
- 未经用户明确确认，不要自动执行 `git commit`、`git push`、创建 GitHub Release、上传安装包或更新 `latest.yml`。
- 可以在本地实现、测试、构建临时产物；但正式发布动作必须单独确认。
- 如果用户只说“做好”“修一下”“直接做”，默认含义是完成本地代码和验证，不等于授权发布到 GitHub。
