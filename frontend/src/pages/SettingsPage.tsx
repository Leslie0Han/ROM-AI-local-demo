...
import { DownloadCloud, Loader2, Save, ShieldCheck } from 'lucide-react';
...
export function SettingsPage() {
  ...
  async function saveDeepSeekSettings() {
    ...
    try {
      ...
      setStatus(updated);
      setApiKey('');
      setMessage('DeepSeek 配置已保存。启动分析将优先使用真实 API。');
    } catch (error) {
        ...
        {status && (
          <div className="space-y-5">
            <div className="rounded-lg border border-[#333333] bg-[#111111] p-5">
              <div className="mb-5 flex items-center gap-2 text-sm font-semibold text-white"><ShieldCheck size={18} className="text-emerald-300" />DeepSeek 配置</div>
